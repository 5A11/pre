...
use crate::state::{
    get_all_available_proxy_pubkeys, get_all_proxies, get_data_entry, get_state, DataEntry, State,
};
...
fn test_add_data() {
    ...
    assert_eq!(
        &get_data_entry(deps.as_mut().storage, &data_id).unwrap(),
        &data_entry
    );
}
...
fn test_proxy_unregister_with_requests() {
    ...
    let delegation1 = Delegation {
        delegator_addr: delegator1.clone(),
    ...
    let delegation3 = Delegation {
        delegator_addr: delegator2.clone(),
    ...
    // Delegation 1 - Only removed proxy 2
    ...
    .is_some());
    assert!(get_delegation(deps.as_mut().storage, &0).is_some());
    ...
    // delgator1, delegatee1, proxy3 - Unaffected
    assert!(get_delegation_id(
        deps.as_mut().storage,
        &delegator1,
        &delegator1_pubkey,
    ...
    .is_some());
    assert!(get_delegation(deps.as_mut().storage, &2).is_some());
    assert!(is_proxy_delegation(
        deps.as_mut().storage,
        &proxy3_pubkey,
        &2
